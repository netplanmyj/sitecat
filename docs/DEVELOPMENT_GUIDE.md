...
1. **普段使い（リクエスト節約）**: `mode: edit` または `chat` ＋ `model: gpt-4o`  
   - これで 90% の作業はこなせます。

2. **複雑な設計・リファクタリング**: `mode: edit` ＋ `model: auto` (または Claude 3.5 Sonnet)  
   - 「自力では難しい」と感じた時だけ、Edit モードのままモデルのランクを上げます。

3. **Agent モードは「自動でテストを回してほしい時」だけ**:  
   - Agent モードは非常に便利ですが、1,500件の枠は Agent モードを常用すると数日で溶けてしまいます。