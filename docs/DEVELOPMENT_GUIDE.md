...
### 1.5 Site数の上限管理（高並行対策）
- 方式: `/users/{uid}/meta/sitesCounter` に原子的なカウンタを保持
- 作成時: Transaction内でカウンタ読取→上限チェック→`FieldValue.increment(1)`→`sites`ドキュメント作成を同一トランザクションで実施
- 削除時: Transaction内で`sites`削除→`FieldValue.increment(-1)`を同一トランザクションで実施
- 初期化: カウンタ未存在時のみTransaction内で現行`sites`件数を一度集計して初期化
- 効果: レース状態を排除し、同時作成でも上限超過の一時的な発生を防止