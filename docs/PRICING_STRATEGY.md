...
2. **サイト数制限解除**（#196）
   - 定数変更（`lib/constants/app_constants.dart`）
   - 無料版3個 → プレミアム30個
   - UI側で課金状態チェック
...
### 買い切り版: ¥1,200
- **ターゲット**: 個人開発者、小規模～中規模サイト運営者
- **競合比較**: 類似ツールの月額料金2-3ヶ月分相当
- **提供価値**: 
  - サイト数30個（無料版の10倍、個人で管理できる現実的な上限）
  - スキャン可能ページ数1000（無料版の5倍、中規模サイト対応）
  - 100ページずつのバッチ処理（最大10回のContinueで完了、UX最適化）
  - 除外パス設定（柔軟な監視）
  - 履歴50件表示（十分な振り返り期間）
...
### 更新履歴
- 2025-12-04: Phase 3a実装に向けたページ制限の再設計
  - 無料版: 100→200ページ（バッチ100 × 2回）
  - Premium版: 1000ページ新設定（バッチ100 × 10回）
  - バッチサイズ: 50→100ページ（UX改善）
  - サイト登録数を実装に合わせて更新（無料版3個）
- 2025年11月: v1.0.3リリース
  - Phase 1/2完了を反映
  - 買い切り・サブスクの2段階戦略を明確化
