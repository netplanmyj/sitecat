...
- **checkSiteLinksメソッド**: 約355行（行65〜420）
  - ⚠️ [旧仕様まで] Step 1: sitemap読み込み（60行）
  - ⚠️ [旧仕様まで] Step 2: ページスキャン・リンク抽出（90行）
  - ⚠️ [旧仕様まで] Step 3: 内部リンクチェック（50行）
  - ⚠️ [旧仕様まで] Step 4: 外部リンクチェック（50行）
  - ⚠️ [旧仕様まで] Step 5: 結果マージ（5行）
  - ⚠️ [旧仕様まで] Step 6: 結果保存（100行）
  - 📝 [新仕様以降] ページ単位での抽出→検証ループに変更予定
