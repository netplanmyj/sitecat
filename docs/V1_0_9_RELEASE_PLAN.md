...
#### Task 3.1: Build method アンチパターン修正 - Issue #313 **IN PROGRESS** 🔄
**優先度**: P2  
**工数**: 2日  
**ステータス**: 🔄 **進行中** - Provider同期を `initState()` に移動中
...
- 🔄 `purchase_screen.dart`: Race condition 修正
...
**次のステップ:**
1. `sites_screen.dart` の修正完了
2. `purchase_screen.dart` の race condition 修正
3. `purchase_provider.dart` 削除
4. 全409テスト実行・確認
5. 実機動作確認
